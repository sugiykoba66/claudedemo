...
`next build` 中はページデータ収集のためにサーバモジュールがすべて評価される。CI runner には `DATABASE_URL` / `SESSION_SECRET` が設定されていないので、何もしないと CI のビルドが必ず落ちる。

このため、`process.env.NEXT_PHASE === 'phase-production-build'` のときに限ってスキーマ検証をスキップし、スキーマを満たすダミー値を採用する（実リクエスト処理ではないため接続できなくても問題ない）。

実行時（standalone 起動 = `node server.js`）には `NEXT_PHASE` が立たないため、本番ランタイムの fail-fast はそのまま機能する。
...
| リソース | SKU / 設定例 |
|---|---|
| App Service Plan (Linux) | B1 以上（現状 `asp-sgclaudedemo`） |
| App Service (`sgclaudedemo`) | Node 20 LTS（`linuxFxVersion=NODE\|20-lts`）/ japaneast |
| Azure SQL Server / Database | S0 以上、SQL 認証 |

公開 URL: <https://sgclaudedemo.azurewebsites.net>

> **Always On は OFF で運用**（コスト最適化）。アイドル中は App Service がコンテナを停止するため、初回アクセスでコールドスタートが発生する（数秒〜数十秒）。ビジネス要件で常時応答が必要になった場合のみ `az webapp config set --always-on true` で有効化する。
...
### 11.5 production 承認ワークフロー（設定済み）

main への push があってもデプロイを即座に走らせない運用。以下は GitHub Settings から手作業で設定済み。

1. **Settings → Environments** に `production` を作成
2. **Required reviewers** に承認者を 1 名以上設定
3. **Deployment branches** を `main` のみに限定
4. **Branch protection rules**（main）で `verify` ジョブを必須化、PR 経由マージのみ許可

main push 後の deploy job は approval 待ちで停止し、承認者の操作で初めてデプロイが進む。承認権限を絞ることで「main へ merge した瞬間に本番反映」を防ぎ、デプロイ意思を明示化している。

設定 URL:
- <https://github.com/sugiykoba66/claudedemo/settings/branches>
- <https://github.com/sugiykoba66/claudedemo/settings/environments>
...
4. Kudu (SCM Basic Auth が必要) コンソールから一回だけ実行:
   ...
   ```
5. Startup Command は **standalone 構成のため `node server.js`**（`output: 'standalone'` のビルド成果物に同梱されるエントリ）。`npm start` を指定すると `next start` が呼ばれ、standalone バンドルからは起動できないので注意
