...
## Branch protection / Environment 設定（GUI 設定済み）

以下は GitHub Settings から手作業で設定済み（Workflow ファイルでは設定できない項目）。リポジトリを fork した場合や別環境に複製する場合は同等の設定が必要。

### Branch protection rules（main）
- `Require a pull request before merging` 有効化
- `Require status checks to pass before merging` 有効化、`verify` ジョブを必須化
- 直 push 不可、PR 経由のみ

設定 URL: <https://github.com/sugiykoba66/claudedemo/settings/branches>

### Environments → production
- `Required reviewers` に承認者を 1 名以上設定（main マージ後の deploy job が approval 待ちで停止する）
- `Deployment branches` を `main` 限定にしてフィーチャーブランチからの誤デプロイを防止

設定 URL: <https://github.com/sugiykoba66/claudedemo/settings/environments>
